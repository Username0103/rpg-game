...
def _get_path(file: str = 'config.ini'):
...
def _check_presence(to_check: tuple[str, dict]) -> bool:
    try:
        ...
        raise KeyError(f'received wrong section write_config(),\
                        was {to_check[0]}') from exc
    return True

def write_config(to_write: tuple[str, dict]):
...
def get_dict_section(section):
    config = _read_config()
    return dict(config[section])

def get_config(to_get: tuple[str, str]):
    config = _read_config()
